...
if __name__ == '__main__':
    ...
    for i, file in enumerate(files):
        ...
        if i == 0:
            summary = analysis_icp(file)[2: 4]
            index = [file_name]
        else:
            summary = np.vstack((summary, analysis_icp(file)[2: 4]))
            index.append(file_name)

    summary_all = pd.DataFrame(summary, index=index, columns=['mean_error', 'average_count'])
    if os.path.exists('summary_all.csv') is True:
        os.remove('summary_all.csv')
    summary_all.to_csv('summary_all.csv')
    print(summary_all)
...
# draw plt_bar
    ...
    bar_with = 0.35
    plt.bar(n_group, 1-summary_all['mean_error'], bar_with, label="accurary")
